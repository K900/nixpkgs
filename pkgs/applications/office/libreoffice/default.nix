...
, symlinkJoin
, postgresql
# Fonts are used in tests, we need these exact fonts to get the exact metrics
, makeFontsConf
, amiri
, caladea
, carlito
...
in stdenv.mkDerivation (finalAttrs: {
  ...
  postPatch = ''
    ...
    # testOutlineLevel fails, skipped for now
    # FIXME: investigate
    substituteInPlace sw/Module_sw.mk --replace-fail CppunitTest_sw_htmlimport ""
  '';
  ...
  postInstall = ''
    ...
    . ./bin/get_config_variables PRODUCTVERSION PRODUCTVERSIONSHORT
    for file in *.desktop; do
	    sed -i -e "s/ *$PRODUCTVERSION//g" -e "s/$PRODUCTVERSIONSHORT//g" $file
    done
  '';
  ...
  disallowedRequisites = lib.concatMap (x: lib.optional (x?dev) x.dev) finalAttrs.buildInputs;
