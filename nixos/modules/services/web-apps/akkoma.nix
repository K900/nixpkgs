...
let
  ...
  staticFiles = pkgs.runCommandLocal "akkoma-static" { } ''
    ${optionalString (cfg.termsOfService != null) ''
      mkdir -p $out/static
      cp --reflink=auto ${escapeShellArg cfg.termsOfService} $out/static/terms-of-service.html
    ''}
  ...
  options = {
    services.akkoma = {
      ...
      termsOfService = mkOption {
        ...
        default = null;
        description = mdDoc "Path of Terms of Service file.";
      };
      ...
      emojiPacks = mkOption {
        description = mdDoc "Emoji packs.";
        type = with types; nullOr (attrsOf package);
        default = null;
        example = literalExpression ''
          {
            "blobs.gg" = pkgs.akkoma-emoji.blobs_gg;
          }
        '';
      };
              ...
              ":instance" = {
                ...
                static_dir = mkOption {
                  ...
                  defaultText = literalMD ''
                    ...
                    - {option}`config.termsOfService`
                    - {option}`config.frontends`
                    - {option}`config.emojiPacks`
                  '';
                  description = mdDoc ''
  ...
  config = mkIf cfg.enable {
    ...
    in {
      ...
      serviceConfig = {
        ...
        BindReadOnlyPaths = mkIf isConfined (mkMerge [
          [ "/etc/hosts" "/etc/resolv.conf" "${staticDir}:${staticDir}:norbind" ]
